  ...
  return (
    <div className={`relative shrink-0 ${className}`} ref={dropdownRef}>
      <button
        ...
        className={buttonStyle || "flex items-center justify-between gap-1 px-2.5 py-2.5 border border-gray-300 rounded-lg bg-gray-50 hover:bg-gray-100 text-gray-800 font-semibold text-[13px] transition-all cursor-pointer min-w-[82px] shadow-sm"}
      >
        ...
        <div className="absolute left-0 top-full mt-1 z-[200] w-52 max-h-52 overflow-y-auto bg-white border border-gray-200 rounded-xl shadow-2xl p-1.5 text-[12px]">
          <div className="p-1 mb-1 border-b border-gray-100 sticky top-0 bg-white z-10">
            <input
              ...
              className="w-full px-2 py-1 text-[11px] border border-gray-200 rounded focus:outline-none focus:border-brand-primary bg-gray-50 text-gray-800"
              autoFocus
            />
          </div>
          {filtered.length === 0 ? (
