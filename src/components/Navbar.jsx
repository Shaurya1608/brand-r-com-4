...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 transition-all duration-300">
      {/* Top Strip */}
      <div className="bg-gradient-to-r from-brand-primary-hover via-brand-primary to-brand-primary-hover w-full py-2 md:py-1.5 px-3 md:px-8 flex flex-col xl:flex-row justify-between items-center text-[9px] sm:text-[10px] md:text-[11px] font-bold tracking-wider sm:tracking-widest md:tracking-[0.2em] text-white shadow-md z-50">
        <div className="hidden xl:block flex-1"></div>
        <div className="text-center xl:flex-shrink-0 mb-2 xl:mb-0 uppercase flex flex-wrap justify-center items-center gap-x-2 sm:gap-x-3 md:gap-x-4 lg:gap-x-5 gap-y-1 w-full xl:w-auto">
          <span className="whitespace-nowrap">EARLY BIRD LIVE</span>
          <span className="font-normal text-white/50 hidden min-[360px]:inline-block">•</span>
          <span className="whitespace-nowrap">AWARDS OPEN</span>
          <span className="font-normal text-white/50 hidden sm:inline-block">•</span>
          <span className="whitespace-nowrap">SPONSORSHIP OPEN</span>
        </div>
        <div className="flex-1 flex justify-center xl:justify-end font-mono whitespace-nowrap text-[11px] md:text-sm w-full xl:w-auto mt-0.5 xl:mt-0">
      ...
      {/* Main Navbar */}
      <div className="w-full bg-brand-surface/90 backdrop-blur-xl border-b border-brand-primary/10 py-2 md:py-3 px-6 md:px-12 shadow-sm">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          
          {/* Logo */}
          <div className="flex-shrink-0 group cursor-pointer">
            <Link href="/" className="flex items-center">
              <img src="/logo/brand-r-comm-logo.png" alt="Brand R.Comm 2026 Logo" className="h-8 md:h-10 lg:h-12 w-auto object-contain transition-transform duration-300 group-hover:scale-105" />
            </Link>
          </div>
